...
@section('title', 'Presensi ' . ($assignment->subject->name ?? ''))

@section('content')

    <div class="d-flex justify-content-between align-items-center mb-3">
        <div>
            <h4 class="mb-0">Presensi — {{ $assignment->subject->name ?? '-' }}</h4>
            <div class="text-muted small">Kelas {{ $assignment->schoolClass->name ?? '-' }}</div>
        </div>
        <a href="{{ route('guru.attendance.index') }}" class="btn btn-outline-secondary">← Kembali</a>
    </div>
    ...
    <div class="card shadow-sm border-0 mb-3">
        <div class="card-body">
            <form method="GET" action="{{ route('guru.attendance.create', $assignment) }}" class="row g-3 align-items-end">
                <div class="col-md-3">
    ...
    <div class="card shadow-sm border-0">
        <div class="card-body">
            ...
            @else
                <form method="POST" action="{{ route('guru.attendance.store', $assignment) }}">
                    @csrf
